...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  __name: "CampusGraphCanvas",
  props: {
    graph: { default: void 0 },
    title: { default: "校园路网图" },
    heightRpx: { default: 520 },
    showRouteText: { type: Boolean, default: true }
  },
  setup(__props) {
    const props = __props;
    ...
    const canvasHeight = common_vendor.computed(() => Math.max(220, Math.floor(props.heightRpx / 750 * systemInfo.windowWidth)));
    const canvasId = `campus_graph_${Math.random().toString(36).slice(2, 8)}`;
    const nowMs = common_vendor.ref(Date.now());
    const selectedRiderId = common_vendor.ref(null);
    const canvasRect = common_vendor.ref(null);
    const riderHitAreas = common_vendor.ref([]);
    let tickTimer;
    const edgeLabel = (edge) => `${edge.distanceM || 0}m/${edge.costTimeSec || 0}s`;
    const graphNodes = common_vendor.computed(() => {
      var _a;
      const nodes = ((_a = props.graph) == null ? void 0 : _a.nodes) || [];
      if (!nodes.length) {
        return [];
      }
      ...
      const padding = 30;
      const toX = (lng) => padding + (lng - minLng) / (maxLng - minLng) * (width - padding * 2);
      const toY = (lat) => height - padding - (lat - minLat) / (maxLat - minLat) * (height - padding * 2);
    ...
    const nodeCoordMap = common_vendor.computed(() => {
    ...
    const dedupEdges = common_vendor.computed(() => {
      var _a;
      const map = /* @__PURE__ */ new Map();
      (((_a = props.graph) == null ? void 0 : _a.edges) || []).forEach((edge) => {
        if (edge.fromNodeId === void 0 || edge.toNodeId === void 0)
          return;
    ...
    const edgeCostMap = common_vendor.computed(() => {
      var _a;
      const map = /* @__PURE__ */ new Map();
      (((_a = props.graph) == null ? void 0 : _a.edges) || []).forEach((edge) => {
        if (edge.fromNodeId === void 0 || edge.toNodeId === void 0)
        ...
        const key = `${from}-${to}`;
        const exists = map.get(key);
        if (exists === void 0 || cost < exists) {
          map.set(key, cost);
        }
      });
      return map;
    });
    const routeMetrics = common_vendor.computed(() => {
      var _a;
      const ids = (((_a = props.graph) == null ? void 0 : _a.routeNodeIds) || []).map((id) => Number(id)).filter((id) => !Number.isNaN(id));
      const segments = [];
      if (ids.length < 2) {
    ...
    const liveElapsedSec = common_vendor.computed(() => {
      ...
      const baseElapsed = (serverMs - startMs) / 1e3;
      const drift = Math.max(0, (nowMs.value - serverMs) / 1e3);
      return baseElapsed + drift;
    });
    const focusRiderId = common_vendor.computed(() => {
      var _a, _b;
      if (((_a = props.graph) == null ? void 0 : _a.riderId) !== void 0 && props.graph.riderId !== null) {
        return Number(props.graph.riderId);
      }
      const hit = (((_b = props.graph) == null ? void 0 : _b.riders) || []).find((rider) => Number(rider.highlight || 0) === 1);
      return (hit == null ? void 0 : hit.riderId) === void 0 ? -1 : Number(hit.riderId);
    });
    const riderRenderList = common_vendor.computed(() => {
      var _a, _b;
      const riders = ((_a = props.graph) == null ? void 0 : _a.riders) || [];
      const nodeMap = nodeCoordMap.value;
      const list = [];
      const dynamicRiderId = focusRiderId.value;
      if (Number((_b = props.graph) == null ? void 0 : _b.dispatchStatus) === 1 && dynamicRiderId > 0 && routeMetrics.value.segments.length) {
      ...
      riders.forEach((rider) => {
        ...
        if (riderId === dynamicRiderId && Number((_a2 = props.graph) == null ? void 0 : _a2.dispatchStatus) === 1) {
          return;
        }
      ...
      grouped.forEach((items, nodeId) => {
        ...
        if (!node)
          return;
        items.slice().sort((a, b) => Number(a.riderId || 0) - Number(b.riderId || 0)).forEach((rider, idx) => {
    ...
    const selectedRiderInfo = common_vendor.computed(() => {
    ...
    common_vendor.watch(
      () => {
        var _a;
        return (_a = props.graph) == null ? void 0 : _a.orderId;
      },
      () => {
        selectedRiderId.value = null;
      }
    );
    common_vendor.watch(riderRenderList, (list) => {
      if (selectedRiderId.value === null)
        return;
    ...
    const drawHouse = (ctx, x, y, nodeType, highlight) => {
    ...
    const drawMoto = (ctx, rider, selected) => {
      const x = rider.x;
      const y = rider.y;
    ...
    const drawPopupOnCanvas = (ctx, rider) => {
      const width = 170;
      const height = 82;
    ...
    const drawGraph = () => {
      ...
      const routeNodeSet = new Set(((graph == null ? void 0 : graph.routeNodeIds) || []).map((id) => Number(id)));
      nodes.forEach((node) => {
    ...
    const refreshCanvasRect = () => {
      return new Promise((resolve) => {
        ...
        query.select(".campus-canvas").boundingClientRect((rect) => {
    ...
    const onCanvasTouch = async (e) => {
      var _a, _b;
      const touch = ((_a = e.changedTouches) == null ? void 0 : _a[0]) || ((_b = e.touches) == null ? void 0 : _b[0]);
      if (!touch) {
        return;
      }
      if (!canvasRect.value) {
      ...
      common_vendor.nextTick$1(() => drawGraph());
    };
    common_vendor.watch(
    ...
    common_vendor.onMounted(() => {
      ...
      common_vendor.nextTick$1(() => refreshCanvasRect());
    });
    ...
    return (_ctx, _cache) => {
      ...
      return common_vendor.e({
        ...
        b: canvasId,
        c: common_vendor.unref(canvasWidth),
        d: canvasHeight.value,
        e: `${common_vendor.unref(canvasWidth)}px`,
        f: `${canvasHeight.value}px`,
        g: common_vendor.o(onCanvasTouch),
        h: _ctx.showRouteText
      }, _ctx.showRouteText ? {
        i: common_vendor.t(((_a = _ctx.graph) == null ? void 0 : _a.routeText) || "-")
      } : {}, {
        j: selectedRiderInfo.value
      }, selectedRiderInfo.value ? {
        k: common_vendor.t(selectedRiderInfo.value.riderName),
        l: common_vendor.t(selectedRiderInfo.value.badge),
        m: common_vendor.t(selectedRiderInfo.value.riderPhone || "-"),
        n: common_vendor.t(selectedRiderInfo.value.activeLoad),
        o: common_vendor.t(selectedRiderInfo.value.nodeName),
        p: common_vendor.t(selectedRiderInfo.value.isMoving ? "行进中" : "待命/驻留")
      } : {}, {
        q: riderRenderList.value.length
      }, riderRenderList.value.length ? {
        r: common_vendor.f(riderRenderList.value, (item, k0, i0) => {
          return {
            a: common_vendor.t(item.badge),
            b: common_vendor.t(item.riderName),
            c: common_vendor.t(item.activeLoad),
            d: common_vendor.t(item.nodeName),
            e: `rider-${item.riderId}`,
            f: item.highlight === 1 ? 1 : ""
          };
        })
      } : {});
    };
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-d3af2771"], ["__file", "D:/new1/hanye-take-out/hanye-take-out-uniapp/src/components/campus/CampusGraphCanvas.vue"]]);
